...
class _NisitFormPageState extends State<NisitFormPage> {
  ...
  final List<String> prefixes = ["นาย", "นางสาว"];
  ...
  String major =
      majorController.text.replaceAll(RegExp(r"\s+"), "").toUpperCase();
  ...
  try {
    bool success = await StudentService.insertStudent(student);
    ...
    if (success) {
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(
          builder: (_) => MainNavigation(email: emailController.text),
        ),
      );
    } else {
      _showDialog(
